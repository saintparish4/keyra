...
/** Unit tests for the pure TokenBucket module.
  *
  * These tests exercise the algorithm directly, without any IO or storage layer.
  * They cover edge cases that are difficult to observe through the store API
  * (e.g. zero elapsed time, near-zero refill rates) and document the invariants
  * stated in the TokenBucket scaladoc.
  */
class TokenBucketSpec extends AnyFreeSpec with Matchers:
  ...
  "TokenBucket.refill" - {
    ...
    "should add zero tokens when elapsed time is 0ms" in {
      ...
      val state  = TokenBucketState(tokens = 5.0, lastRefillMs = 1000L, version = 1L)
    ...
    "should produce negligible refill for a near-zero refill rate" in {
      ...
      val slowProfile =
        RateLimitProfile(capacity = 10, refillRatePerSecond = 0.001, ttlSeconds = 3600)
      val state  = TokenBucketState(tokens = 0.0, lastRefillMs = 0L, version = 1L)
    ...
    "RateLimitProfile should reject a zero refill rate at construction" in {
      an[IllegalArgumentException] should be thrownBy {
        RateLimitProfile(capacity = 10, refillRatePerSecond = 0.0, ttlSeconds = 3600)
      }
    }
    ...
    "RateLimitProfile should reject zero capacity at construction" in {
      an[IllegalArgumentException] should be thrownBy {
        RateLimitProfile(capacity = 0, refillRatePerSecond = 1.0, ttlSeconds = 3600)
      }
    }
  }
  ...
  "TokenBucket.consume" - {
    ...
    "should succeed and deduct exactly cost tokens" in {
      val state  = TokenBucketState(tokens = 10.0, lastRefillMs = 0L, version = 1L)
    ...
    "should increment version exactly once per successful consume" in {
      val state  = TokenBucketState(tokens = 10.0, lastRefillMs = 0L, version = 4L)
  ...
  "TokenBucket.retryAfterSeconds" - {
    ...
    "should calculate wait proportional to deficit" in {
      // ceil((10 - 0) / 1.0) = 10
      val result = TokenBucket.retryAfterSeconds(cost = 10, tokens = 0.0, profile)
      result shouldBe 10
    }
    ...
    "should scale with refill rate" in {
      ...
      val fastProfile =
        RateLimitProfile(capacity = 100, refillRatePerSecond = 10.0, ttlSeconds = 3600)
      val result = TokenBucket.retryAfterSeconds(cost = 5, tokens = 0.0, fastProfile)
      result shouldBe 1
    }
  }
