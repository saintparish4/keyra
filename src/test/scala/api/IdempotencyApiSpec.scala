...
/** Unit tests for IdempotencyApi: TTL capping and warning when client TTL exceeds max.
  */
class IdempotencyApiSpec extends AsyncFreeSpec with AsyncIOSpec with Matchers:
  ...
  "IdempotencyApi TTL capping" - {
    ...
    "should pass capped TTL to store when client requests TTL above max" in {
      ...
      val config = IdempotencyConfig(defaultTtlSeconds = 86400, maxTtlSeconds = maxTtl)
      ...
      val test = for
        ...
        store = new IdempotencyStore[IO]:
          override def check(
            ...
            capturedTtl.set(Some(ttlSeconds)) *>
              Clock[IO].realTime.map(d => IdempotencyResult.New(idempotencyKey, Instant.ofEpochMilli(d.toMillis)))
  ...
  private def capturingLogger(warnLogs: Ref[IO, List[String]]): Logger[IO] =
    new Logger[IO]:
      ...
      override def warn(t: Throwable)(message: => String): IO[Unit] = warnLogs.update(_ :+ message)
      override def warn(message: => String): IO[Unit] = warnLogs.update(_ :+ message)
