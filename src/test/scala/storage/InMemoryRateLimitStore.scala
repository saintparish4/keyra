    ...
    for
      ...
      decision <- stateRef.modify { stateMap =>
        val current = stateMap
          .getOrElse(key, TokenBucketState(profile.capacity.toDouble, nowMs, 0L))
...
object InMemoryRateLimitStore:
  def create[F[_]: Sync]: F[InMemoryRateLimitStore[F]] = Ref
    .of[F, Map[String, TokenBucketState]](Map.empty)
    .map(new InMemoryRateLimitStore[F](_))
