...
class InMemoryRateLimitStoreSpec
  ...
  "InMemoryRateLimitStore" - {
    ...
    "should always reject when cost exceeds bucket capacity" in {
      ...
      val capacityProfile = RateLimitProfile(
        capacity = 5,
      ...
      val test =
        for
          store <- InMemoryRateLimitStore.create[IO]
          decision <- store
            .checkAndConsume("edge-key", cost = 10, capacityProfile)
        yield decision
    ...
    "tokens remaining should never be reported as negative" in {
      ...
      val test =
        for
          store <- InMemoryRateLimitStore.create[IO]
          _ <- (1 to 10).toList.traverse(_ =>
            store.checkAndConsume("neg-key", cost = 1, testProfile),
          )
          decisions <- (1 to 5).toList.traverse(_ =>
            store.checkAndConsume("neg-key", cost = 1, testProfile),
          )
        yield decisions
      ...
      test.asserting(decisions =>
        forEvery(decisions) { d =>
          ...
          d.asInstanceOf[RateLimitDecision.Rejected].retryAfterSeconds should
            be >= 1
        },
      )
    }
