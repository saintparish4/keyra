...
object TracingMiddleware:
  ...
  def apply[F[_]: Async: Tracer](routes: HttpRoutes[F]): HttpRoutes[F] =
    Kleisli { (req: Request[F]) =>
      ...
      OptionT.liftF {
        tracer.spanBuilder(spanName).addAttribute(
          org.typelevel.otel4s.Attribute("http.method", req.method.name),
        ).addAttribute(org.typelevel.otel4s.Attribute(
          "http.target",
          req.pathInfo.renderString,
        )).withSpanKind(SpanKind.Server).build.use(span =>
          routes(req).semiflatMap(resp =>
            span.addAttribute(
              org.typelevel.otel4s
                .Attribute("http.status_code", resp.status.code.toLong),
            ) *>
              (if resp.status.isSuccess then Async[F].unit
               else span.setStatus(StatusCode.Error)) *> Async[F].pure(resp),
          ).getOrElseF(
  ...
  /** Create a child span for a named operation. Use inside service methods:
    ...
    * }}}
    */
  def traced[F[_]: Tracer: Async, A](name: String)(fa: F[A]): F[A] = Tracer[F]
    .span(name).use(_ => fa)
