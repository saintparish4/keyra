...
object AggregateHealth:
    given Encoder[ComponentHealth] = Encoder.forProduct3(
        "name", "status", "details"
    )(c => (c.name, c.status, c.details))
    given Encoder[AggregateHealth] = Encoder.forProduct2(
        "status", "components"
    )(a => (a.status, a.components))
...
object HealthAggregator:
    ...
    def aggregate[F[_]: Temporal](
        ...
        sources.traverse(_.check).map { components =>
    ...
    def dynamoDbSource[F[_]: Temporal](
      ...
      new HealthSource[F]:
        ...
        def check: F[ComponentHealth] =
          healthCheck.handleError(e => Left(e.getMessage)).map {
            case Right(_) => ComponentHealth(sourceName, "ok")
            case Left(err) => ComponentHealth(sourceName, "error", Some(Json.fromString(err)))
          }
    ...
    def circuitBreakerSource[F[_]: Temporal](
        ...
        def check: F[ComponentHealth] = cb match
            case Some(breaker) => breaker.state.map { s =>
                ComponentHealth(name, s.toString.toLowerCase,
    ...
    def kinesisSource[F[_]: Temporal](
        ...
        def check: F[ComponentHealth] =
            healthCheck.handleError(e => Left(e.getMessage)).map {
                ...
                case Left(err) => ComponentHealth(name, "error", Some(Json.fromString(err)))
            }